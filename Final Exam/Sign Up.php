...
<form action="" method="post" name="SignUp" onsubmit="return PasswordMatch()">
      ...
      <label class="signupinfo">Password:   </label><input type="password" name="pwd1" size="30" pattern="(?=. *d)(?=. *[a-z])(?=. *[A-Z])(?=.*?[#?!@$%^&*-\=+]\[]).{8,}" title="Must contain at least a number, one uppercase letter, one lowercase letter, and at least 8 or more characters" placeholder="Password" required><br><br>
