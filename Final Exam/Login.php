...
<form class="lgform" action="" method="post" >
   <label class="login-info" >Email: </label><input type="text" name="email" size="60" pattern="/^\w+([\.-]?\w+)@\w+([\.-]?\w+)(\.\w{2,3})+$/" placeholder="Email" required/><br><br><br>
   <label class="login-info" >Password: </label><input type="password" name="password" size="60" pattern="(?=. *d)(?=. *[a-z])(?=. *[A-Z])(?=.*?[#?!@$%^&*-\=+]\[]).{8,}" placeholder="Password" required/><br><br><br>
